...
class BinanceOrderBookConnection(HttpPollingOrderBookConnection):
    ...
    def build_tasks(self):
        ...
        for index, group in enumerate(symbol_groups):
            if use_ws:
                ...
                connection = BinanceWsOrderBookConnection(
                    self.exchange_settings, self.store, index + 1, group
                )
            else:
                connection = BinanceOrderBookConnection(
